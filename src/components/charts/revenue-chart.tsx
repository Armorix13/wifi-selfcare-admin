...
const generateRevenueData = (months: number): RevenueData[] => {
...
export function RevenueChart({ title }: RevenueChartProps) {
  ...
  const getMetricConfig = () => {
    switch(metric) {
      case "revenue":
        return {
          dataKey: "revenue" as keyof RevenueData,
          color: "#10b981",
          name: "Revenue ($)",
      ...
      case "customers":
        return {
          dataKey: "customers" as keyof RevenueData, 
          color: "#3b82f6",
      ...
      case "churn":
        return {
          dataKey: "churnRate" as keyof RevenueData,
          color: "#ef4444", 
          name: "Churn Rate (%)",
      ...
      case "arpu":
        return {
          dataKey: "arpu" as keyof RevenueData,
          color: "#f59e0b",
          name: "ARPU ($)",
          format: (value: number) => `$${value}`
        };
  ...
  const currentValue:any = chartData.length > 0 ? chartData[chartData.length - 1][metricConfig.dataKey] : 0;
  const previousValue:any = chartData.length > 1 ? chartData[chartData.length - 2][metricConfig.dataKey] : 0;
      ...
      <div className="mt-4 flex items-center justify-between text-sm text-muted-foreground">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-1">
            ...
            <span>Avg: {metricConfig.format(chartData.reduce((sum:any, item:any) => sum + item[metricConfig.dataKey], 0) / chartData.length)}</span>
          </div>
        </div>
